...
class StrongPixDecoder(PixDecoder):
    ...
    def forward_probs(
        ...
        for i, (rgb_dec, clf, feat_conv) in enumerate(
            ...
            # print(f"Level {i}: probs shape = {probs.shape}, lower shape = {lower.shape}, upper shape = {upper.shape}")  # Debug statement
...
class Compressor(nn.Module):
    ...
    def forward(self,  # type: ignore
        ...
        # Lists to store the averaged NLL and entropy for each resolution level.
        outer_nll = []
        outer_entropy = []
        
        # Process resolution levels from lowest to highest (last iteration is full resolution).
        for dec, ctx_upsampler, x, y, in zip(  # type: ignore
            ...
            channel_nll = []
            channel_entropy = []
            # For each resolution level, dec_bits.probs returns one entry per channel (3 channels).
            
            for level, (y_i, lm_probs, _) in enumerate(dec_bits.probs):
                if lm_probs is not None:
                    nll, entropy = self.loss_fn(y_i, lm_probs.probs)
                    channel_nll.append(nll.mean().item())
                    channel_entropy.append(entropy.mean().item())
            
             # Average over the channels for the current resolution level.
            if channel_nll and channel_entropy:
                avg_nll = np.mean(channel_nll)
                avg_entropy = np.mean(channel_entropy)
            else:
                avg_nll = 0.0
                avg_entropy = 0.0
            outer_nll.append(avg_nll)
            outer_entropy.append(avg_entropy)
            
        return bits, outer_nll, outer_entropy
