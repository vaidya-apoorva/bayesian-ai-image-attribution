...
def min_value(tensor):
    einzigartige_werte, _ = torch.sort(torch.unique(tensor))  # Eindeutige Werte sortieren
    return einzigartige_werte[1] if einzigartige_werte[0] == 0 and len(einzigartige_werte) > 1 else einzigartige_werte[0]
...
class DiscretizedMixLogisticLoss(nn.Module):
    ...
    def forward(  # type: ignore
        ...
        x_vals = torch.arange(256, dtype=torch.float32, device=logit_pis.device)
        ...
        device = means.device
        num_vals = 256  # Anzahl der Werte (0 bis 255)
        log_probs_list = []  # Liste für alle log_probs-Werte

        # Anstatt alles auf einmal zu berechnen, Berechnung von jeden Wert einzeln in einer Schleife
        for i in range(num_vals):
            x_val = torch.tensor(i, dtype=torch.float32, device=device).view(1, 1, 1, 1,
                                                                             1)  # Single-Wert statt großer Tensor

            log_prob_all = self.log_cdf(x_val, x_val, means.unsqueeze(-1), log_scales.unsqueeze(-1))

            log_prob_weighted_all = log_weights.unsqueeze(-1) + log_prob_all
            log_prob_final_all = torch.logsumexp(log_prob_weighted_all, dim=2)
            log_probs_list.append(log_prob_final_all)
            #prob = torch.exp(log_prob_final_all)
            #entropy_sum += -torch.sum(prob * log_prob_final_all, dim=-1)  # Akkumulieren statt große Matrizen zu speichern
        log_probs_all = torch.cat(log_probs_list, dim=-1)  # Alle zusammenfügen
        probs_all = torch.exp(log_probs_all)  # Wahrscheinlichkeiten berechnen
        entropy = -torch.sum(probs_all * log_probs_all, dim=-1)  # Exakte Entropie

        # final log(P), NCHW
        nll = -torch.logsumexp(log_probs_weighted, dim=2) # log Sum(exp(xij), wieder die Summe über K=10
        return nll, entropy
