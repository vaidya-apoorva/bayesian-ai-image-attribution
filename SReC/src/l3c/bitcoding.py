...
class Bitcoding(object):
    ...
    def encode(
        ...
        with torch.no_grad():
            bits, nll, entropy = self.compressor(x)
        ...
        return bits, entropy_coding_bytes, nll, entropy
    ...
    def encode_uniform(self, S: torch.Tensor, levels: int, fout) -> int:
        ...
        if levels == 4:
            S = S.reshape(-1)
            # print(f"S.shape before padding: {S.shape}")  # Debug statement
            padding = (4 - (S.shape[0] % 4)) % 4
            S = F.pad(S, (0, padding))
            # print(f"S.shape after padding: {S.shape}")  # Debug statement
