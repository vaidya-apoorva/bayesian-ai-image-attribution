...
class _PatchedLPIPS(lpips.LPIPS):
    ...
    def forward(self, in0, in1, retPerLayer=False, normalize=False):
        ...
        print("in0 min/max:", in0.min().item(), in0.max().item())
        print("in1 min/max:", in1.min().item(), in1.max().item())
        ...
        print("Inputs after scaling (if applicable):")
        print("in0_input min/max:", in0_input.min().item(), in0_input.max().item())
        print("in1_input min/max:", in1_input.min().item(), in1_input.max().item())
        
        outs0, outs1 = self.net.forward(in0_input), self.net.forward(in1_input)
        print("Feature outputs from net.forward:")
        print("outs0[0] shape:", outs0[0].shape, "min/max:", outs0[0].min().item(), outs0[0].max().item())
        print("outs1[0] shape:", outs1[0].shape, "min/max:", outs1[0].min().item(), outs1[0].max().item())
        ...
        for kk in range(self.L):
            ...
            print(f"Layer {kk}:")
            print("feats0 min/max:", feats0[kk].min().item(), feats0[kk].max().item())
            print("feats1 min/max:", feats1[kk].min().item(), feats1[kk].max().item())
            print("diffs min/max:", diffs[kk].min().item(), diffs[kk].max().item())
        ...
        for layer in range(self.L):
            val += res[layer]
            print(f"Layer {layer} res min/max:", res[layer].min().item(), res[layer].max().item())

        print("Final aggregated value (val) min/max:", val.min().item(), val.max().item())

        if retPerLayer:
            return (val, res_no_up)
        else:
            print("Final value (val) before returning:", val)
            print("val min/max:", val.min().item(), val.max().item())
            return val
...
def _compute_lpips(
    ...
    print("Dataset A size:", len(ds_a))
    print("Dataset B size:", len(ds_b))

    # Debug: Inspect DataLoader batches
    for batch_a, batch_b in zip(dl_a, dl_b):
        print("Batch A shape:", batch_a[0].shape)  # Assuming batch_a is a tuple (images, labels)
        print("Batch B shape:", batch_b[0].shape)
        print("Batch A sample values:", batch_a[0][0, :, :5, :5])  # First image in batch
        print("Batch B sample values:", batch_b[0][0, :, :5, :5])
    ...
    # Debug: Inspect the final LPIPS layers
    for i, lpips_layer in enumerate(lpips_layers):
        print(f"Final LPIPS layer {i} shape:", lpips_layer.shape)
        print(f"Final LPIPS layer {i} values:", lpips_layer)
    return lpips_layers
...
class LPIPS(Distance):
    ...
    def _postprocess(self, result: list[torch.Tensor]) -> dict[str, torch.Tensor]:
        ...
        print("Raw dist_tensor values:", result)
        out = {}
        if self.layer == -1:
            for i, tensor in enumerate(result):
                out[f"lpips_{self.net}_{i}"] = -tensor
                print(f"Negated tensor for layer {i}:", out[f"lpips_{self.net}_{i}"])
        else:
            out[f"lpips_{self.net}_{self.layer}"] = -result[self.layer]
            print(f"Negated tensor for layer {self.layer}:", out[f"lpips_{self.net}_{self.layer}"])

        for layer, tensor in out.items():
            if not self.spatial:
                # out[layer] = tensor.mean((2, 3), keepdim=True)
                out[layer] = tensor.to(dtype=torch.float32).mean((2, 3), keepdim=True)
                print(f"Mean-reduced tensor for layer {layer}:", out[layer])
...
def distance_from_config(
    ...
    **kwargs,
    ...
    if config.startswith("lpips"):
        ...
        print("LPIPS configuration:", config)
        print("Initialized LPIPS object:", distance)
    else:
