    ...
    for transform_config in transforms:
        ...
        for dir in dirs:
            ...
            for repo_id in repo_ids:
                ...
                img_orig, _ = ds[0]
                img_rec, _ = ds_rec[0]
                diff = torch.abs(img_orig - img_rec)
                print("Mean absolute difference between images:", diff.mean().item())
                print("Image original min/max:", img_orig.min().item(), img_orig.max().item())
                print("Image recon min/max:", img_rec.min().item(), img_rec.max().item())
                print("Same image tensor:", torch.equal(img_orig, img_rec))
                
                # Inspect reconstructed image paths
                print("Reconstructed image paths:", rec_paths[:5])  # Print first 5 paths
                ...
                for dist_metric in distance_metrics:
                    ...
                    for dist_name, dist_tensor in dist_dict.items():
                        dist_tensor = dist_tensor.to(torch.float32)
                        print(f"\n--- {dist_name} ---")
                        print("dist_tensor shape:", dist_tensor.shape)

                        print("dist_tensor sample values:", [f"{x:.5f}" for x in dist_tensor.flatten()[:10].tolist()])
