...
def compute_reconstructions(
    ...
    if not (
        ...
        for images, paths in tqdm(
            ...
            # Debug: inspect batch shape and range before normalization
            print(f"Batch size: {images.shape[0]}")
            print(f"Image tensor dtype before normalization: {images.dtype}")
            print(f"Image min/max before normalization: {images.min().item()}/{images.max().item()}")

            # normalize
            images = images.to(device(), dtype=ae.dtype) * 2.0 - 1.0

            # Debug: inspect batch after normalization
            print(f"Image min/max after normalization: {images.min().item()}/{images.max().item()}")
            print(f"Image dtype after normalization: {images.dtype}")
            ...
            try:
                ...
                print(f"Reconstruction shape: {reconstructions.shape}, dtype: {reconstructions.dtype}")
            except Exception as e:
            ...
            # Debug: inspect output range
            print(f"Reconstruction min/max after denorm: {reconstructions.min().item()}/{reconstructions.max().item()}")
