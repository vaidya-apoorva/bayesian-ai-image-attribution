...
class BinaryImageDataset(Dataset):
    def __init__(self, samples: List[Tuple[str, int]], transform=None):
        ...
        binary_label: 1 = positive (target generator), 0 = negative (all others)
        """
...
def train_one_ovr_classifier(
    ...
    print(f"\n===== Training OvR classifier for: {target_class} =====")

    non_target_classes = [c for c in all_classes if c != target_class]

    # ---------- Build train samples ----------
    pos_train = splits[target_class]["train"]
    n_pos = len(pos_train)

    neg_train_pool = []
    for c in non_target_classes:
        neg_train_pool.extend(splits[c]["train"])
    random.shuffle(neg_train_pool)

    if len(neg_train_pool) < n_pos:
        raise ValueError(
            f"Not enough negatives for {target_class}: have {len(neg_train_pool)}, need {n_pos}"
        )

    neg_train = neg_train_pool[:n_pos]  # balanced
    ...
    def build_eval_samples(split_name: str):
        pos = splits[target_class][split_name]
        neg = []
        for c in non_target_classes:
            neg.extend(splits[c][split_name])
    ...
    print(f"[{target_class}] Train: pos={len(pos_train)}, neg={len(neg_train)}, total={len(train_samples)}")
    print(f"[{target_class}] Val:   {len(val_samples)}")
    print(f"[{target_class}] Test:  {len(test_samples)}")

    # For JSON stats
    stats = {
        "train_pos": len(pos_train),
        "train_neg": len(neg_train),
        "val_pos": len(splits[target_class]["val"]),
        "val_neg": sum(len(splits[c]["val"]) for c in non_target_classes),
        "test_pos": len(splits[target_class]["test"]),
        "test_neg": sum(len(splits[c]["test"]) for c in non_target_classes),
    }
    ...
    print(f"[{target_class}] Test accuracy: {test_acc:.4f}")
    stats["test_accuracy"] = test_acc

    # ---- Save ----
    os.makedirs(output_dir, exist_ok=True)
    base = sanitize_name_for_filename(target_class)
    filename = f"resnet50_{base}_vs_non{base}.pth"
    save_path = os.path.join(output_dir, filename)
    torch.save(model.state_dict(), save_path)
    print(f"[{target_class}] Saved model to: {save_path}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Train generator_vs_nongenerator OvR ResNet50 classifiers "
                    "for GI dataset structure"
    )
    parser.add_argument(
        "--data_root",
        type=str,
    ...
    # We'll train OvR classifiers for GENERATORS only (not coco)
    generator_classes = [c for c in all_classes if c != "coco"]

    print("\nTraining on device:", args.device)
    print("Will train OvR classifiers for generators:", generator_classes)

    model_stats = {}

    for cls in generator_classes:
        stats = train_one_ovr_classifier(
            target_class=cls,
            splits=splits,
        ...
        model_stats[cls] = stats

    # Save JSON with counts per model
    stats_path = os.path.join(args.output_dir, "ovr_model_image_counts.json")
