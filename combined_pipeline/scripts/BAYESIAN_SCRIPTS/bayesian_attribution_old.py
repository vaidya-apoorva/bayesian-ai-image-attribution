...
available_generators = ['coco', 'dall-e2', 'dall-e3', 'firefly', 'midjourneyV5', 'midjourneyV6', 'sdxl', 'stable_diffusion_1-5']
...
class BayesianAttributor:
    ...
    def __init__(self, method='srec', model='imagenet', use_priors=True):
        """
        Initialize Bayesian Attributor

        Args:
            method: 'srec' or 'rigid' - method to use for computing priors
    ...
    def load_binary_classifiers(self):
        ...
        for generator in available_generators:
            ...
            if any('fc.1.' in k for k in fc_keys):
                ...
                logger.debug(f"{generator}: Using Sequential fc structure")
            else:
                # Simple Linear structure
                model.fc = nn.Linear(model.fc.in_features, 2)
                logger.debug(f"{generator}: Using Linear fc structure")
    ...
    def load_rigid_priors(self):
        ...
        ai_exp_vals = np.exp(np.array(ai_norm) * temperature)
    ...
    def get_likelihoods(self, image_path):
        ...
        # Define class mappings based on empirical testing
        # DALL-E2 uses: Class 0 = AI, Class 1 = Real
        # All others use: Class 0 = Real, Class 1 = AI
        correct_mapping_generators = {'dall-e2'}  # Only DALL-E2 has correct mapping
        ...
        with torch.no_grad():
            for generator, model in self.classifiers.items():
                try:
                    ...
                    # Apply correct class mapping based on generator
                    if generator in correct_mapping_generators:
                        # DALL-E2: Class 0 = AI, Class 1 = Real
                        ai_prob = probs[0].item()
                    else:
                        # All others: Class 0 = Real, Class 1 = AI (inverted)
                        ai_prob = probs[1].item()

                    likelihoods[generator] = ai_prob
                    logger.debug(f"{generator}: P(AI) = {ai_prob:.4f}")
        ...
        with torch.no_grad():
            for generator, model in self.classifiers.items():
                try:
                    ...
                    # Apply correct class mapping for real probability
                    if generator in correct_mapping_generators:
                        # DALL-E2: Class 0 = AI, Class 1 = Real
                    ...
                    logger.debug(f"{generator}: P(Real) = {real_prob:.4f}")
                except:
        ...
        logger.debug(f"Combined P(Real) = {real_likelihood:.4f}")
